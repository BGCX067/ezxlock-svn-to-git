...
void SLockCanvas::setEng(bool s)
{
    isEng = s;
}
bool SLockCanvas::getEng()
{
    return isEng;
}
bool SLockCanvas::getcanvasSH()
{
    return isShow;
}

void SLockCanvas::setcanvasSH(bool s)
{
    isShow = s;
    if(s)
    {
...
void SLockCanvas::setLCD(bool s)
{
    isBright = s;
}
...
void SLockCanvas::setapp()
{
    if (ringIcon==1)
    {
        SETUP_SET_Manager cfg("/tmp/ezx_alertstyle.cfg",true);
        cfg.setSettingInfo("ALERTSTYLE_CONFIG_TABLE","AlertSystemVolume",ringVolume);
    }
    killTimers();
    hide();
    isShow = false;
...
//------------------------------------------------------------------
    ...
    {
        int sss = 2;
        heightB = heightB - 1*sss;
        whoSmall = whoSmall - rateF*sss;
...
void SLockCanvas::mousePressEvent(QMouseEvent *e)
{
    ...
    if ((e->x()>(240-27-27))&&(e->x()<240-27)&&(e->y()<15)&&(ringIcon==1))
    {
        ...
        if (speakerOF)
            ...
            cfg.setSettingInfo("ALERTSTYLE_CONFIG_TABLE","AlertSystemVolume",0);
    }
